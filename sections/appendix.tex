...
\label{sec:AppendixB}
  ...
  e.g. Metamask [62]. The main functionality is that the front end is then able to send events through EventBridge [63] to individual lambdas [64] for different 
  functionality. These lambdas can then store data into the database. The payment lambda uses Stripe [65] to process payments, and the \textit{'car lambda'} is
